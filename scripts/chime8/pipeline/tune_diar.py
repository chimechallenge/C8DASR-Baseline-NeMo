...
def sample_params(cfg: DictConfig, trial: optuna.Trial):
    ...
    cfg.diarizer.msdd_model.parameters.sigmoid_threshold = trial.suggest_float("sigmoid_threshold", 0.15, 0.7, step=0.02)
...
def objective(
    ...
    with Path(optuna_output_dir, f"trial-{trial.number}") as output_dir:
        ...
        with torch.cuda.device(gpu_id):
            ...
            tot_der = abs(der)
            logging.info(f"Finished trial: {trial.number}, total DER {tot_der}")
            return tot_der
